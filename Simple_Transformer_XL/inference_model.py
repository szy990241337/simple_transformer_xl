"""
inference_model仅仅用于重构已经训练好的模型图，去掉了label与loss计算
infercnce_model与simple_model是一致的，如果对simple_model做了更改，也必须同时对infercnce_model做修改
"""
...
def mask_adaptive_embedding_lookup(x, n_token, d_embed, d_proj, initializer,
                                   proj_initializer, scope='adaptive_embed'):
    # d_proj 就是 d_model,
    ...
    with tf.variable_scope(scope):
        ...
        if d_proj != d_embed:
            proj_W = tf.get_variable('proj_W', [d_embed, d_proj], # 如果 d_embed与d_model不一样，就无法skip connect了，所以，一旦不一样，强行通过矩阵乘法把词向量的维度弄成d_proj
...
def mask_adaptive_logsoftmax(hidden, n_token, params, scope='adaptive_softmax'):
...
#
def transformer(dec_inp, mems, n_token, n_layer, d_model, d_embed,
    ...
    with tf.variable_scope(scope):
        ...
        if proj_initializer is None:  # pass
        ...
        embeddings, shared_params = lookup_fn(
            ...
            d_proj=d_model,
            initializer=initializer,
            proj_initializer=proj_initializer,)  # shared_params：list:[矩阵一，矩阵二] 矩阵一就是词嵌入矩阵，矩阵二的shape是[d_embed, d_model]
        ...
        logits_out = logsoftmax_fn(hidden=output, n_token=n_token, params=shared_params)
